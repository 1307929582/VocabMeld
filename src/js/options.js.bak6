...
import { API_PRESETS, CEFR_LEVELS } from './core/config.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadSettings() {
    ...
    // API 配置
    elements.apiEndpoint.value = config.apiEndpoint || API_PRESETS.deepseek.endpoint;
    elements.apiKey.value = config.apiKey || '';
    elements.modelName.value = config.modelName || API_PRESETS.deepseek.model;

    // 学习偏好
    elements.nativeLanguage.value = config.nativeLanguage || 'zh-CN';
    elements.targetLanguage.value = config.targetLanguage || 'en';

    const diffIdx = CEFR_LEVELS.indexOf(config.difficultyLevel || 'B1');
    ...
    // 行为设置
    elements.autoProcess.checked = config.autoProcess ?? false;
    elements.showPhonetic.checked = config.showPhonetic ?? true;

    const translationStyle = config.translationStyle || 'translation-original';
  ...
  // 加载词汇列表
  async function loadWordLists(result) {
    ...
    // 加载缓存
    // 使用 storage 服务读取缓存
    const cacheData = await storage.getLocal(['vocabmeld_word_cache']);
    const cache = cacheData.vocabmeld_word_cache || [];
  ...
  async function removeWord(word, type) {
    if (type === 'learned') {
      const config = await storage.getConfig();
      const list = (config.learnedWords || []).filter(w => w.original !== word);
      await storage.set({ learnedWords: list });
      loadSettings();
    } else if (type === 'memorize') {
      const config = await storage.getConfig();
      const list = (config.memorizeList || []).filter(w => w.word !== word);
  ...
  // 加载统计数据
  async function loadStats(result) {
    ...
    // 使用 storage 服务读取缓存
    const cacheData = await storage.getLocal(['vocabmeld_word_cache']);
    const cacheSize = (cacheData.vocabmeld_word_cache || []).length;
  ...
  async function saveSettings() {
    ...
    try {
      ...
      // apiKey 单独存到 local（不跨设备同步）
      await storage.setLocal({ apiKey });
  ...
  function bindEvents() {
    ...
    elements.testConnectionBtn.addEventListener('click', async () => {
      ...
      chrome.runtime.sendMessage({
        ...
        if (response?.success) {
          ...
          elements.testResult.textContent = '✗ ' + (response?.error || response?.message || '连接失败');
    ...
    elements.resetTodayBtn.addEventListener('click', async () => {
      await storage.set({ todayWords: 0 });
      loadSettings();
      debouncedSave(200);
    });
    ...
    elements.resetAllBtn.addEventListener('click', async () => {
      if (confirm('确定要重置所有数据吗？这将清空所有统计和词汇列表。')) {
        ...
        await storage.setLocal({ vocabmeld_word_cache: [] });
        loadSettings();
        debouncedSave(200);
      }
    });
