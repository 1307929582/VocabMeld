...
class ProcessingService {
  ...
  restorePage() {
    ...
    // 恢复所有替换
    textReplacer.restoreAll(document.body);
  ...
  /**
   * 处理特定词汇列表（用于memorizeList触发的定向处理）
   * @param {string[]} words - 目标词汇列表
   * @param {object} options - 选项
   * @returns {Promise<{processed: number, errors: number}>}
   */
  async processSpecificWords(words, options = {}) {
    const targetWords = Array.isArray(words)
      ? [...new Set(words.map(w => (w || '').trim()).filter(Boolean))]
      : [];

    if (targetWords.length === 0) {
      return { processed: 0, errors: 0, empty: true };
    }
    ...
    try {
      ...
      document.querySelectorAll('.vocabmeld-translated').forEach(el => {
        ...
        if (original) alreadyTranslated.add(original.toLowerCase());
      });
      ...
      // 获取白名单
      const whitelist = await storage.getWhitelist();
      const whitelistWords = new Set((whitelist || []).map(w => (w.original || '').toLowerCase()).filter(Boolean));
      ...
      for (const segment of segments) {
        try {
          ...
          for (const w of targetSet) {
            if (alreadyTranslated.has(w)) continue;
            if (whitelistWords.has(w)) continue;

            // 英文词边界匹配，中文包含匹配
            const isAsciiWord = /^[a-z0-9_-]+$/i.test(w);
            if (isAsciiWord) {
              const re = new RegExp(`\\b${w.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
              if (re.test(segment.text)) matched.push(w);
            } else {
          ...
          // 借用现有translate流程（含缓存）
          const replacements = await apiService.translate(segment.text);
          if (!Array.isArray(replacements) || replacements.length === 0) continue;

          const matchedSet = new Set(matched);
          const filtered = replacements
            .filter(r => r?.original && matchedSet.has(r.original.toLowerCase()))
            .filter(r => !whitelistWords.has(r.original.toLowerCase()));

          if (filtered.length === 0) continue;

          // 定向处理不标记为已处理（避免后续全量处理被跳过）
          const count = textReplacer.applyReplacements(segment.element, filtered, {
          ...
          console.error('[VocabMeld] processSpecificWords segment error:', err);
          errors++;
        }
      }
      ...
      console.error('[VocabMeld] processSpecificWords error:', err);
      return { processed, errors: errors + 1, error: err.message };
    } finally {
      this.isProcessing = false;
    }
  }
