...
class TextReplacer {
   ...
   * @param {string} options.translationStyle - 显示样式
   * @param {boolean} options.markProcessed - 是否标记为已处理
   * @returns {number} - 替换数量
   */
  applyReplacements(element, replacements, options = {}) {
    ...
    const translationStyle = options.translationStyle || 'translation-original';
    ...
    for (const replacement of sortedReplacements) {
      ...
      for (const textNode of textNodes) {
        if (this.replaceInTextNode(textNode, original, translation, phonetic, difficulty, translationStyle)) {
          count++;
          break; // 每个替换项只替换一次
        }
      }
   ...
   * @param {string} phonetic - 音标
   * @param {string} difficulty - 难度
   * @param {string} translationStyle - 显示样式
   * @returns {boolean}
   */
  performReplacement(textNode, startIndex, original, translation, phonetic, difficulty, translationStyle) {
    try {
      ...
      // 创建替换元素 - 使用新方法
      const wrapper = this.createWrapperElement(original, translation, phonetic, difficulty, translationStyle);
  ...
  /**
   * 创建替换元素（支持translationStyle，避免innerHTML注入）
   * @param {string} original - 原词
   * @param {string} translation - 翻译
   * @param {string} phonetic - 音标
   * @param {string} difficulty - 难度
   ...
   * @returns {HTMLElement}
   */
  createWrapperElement(original, translation, phonetic, difficulty, translationStyle = 'translation-original') {
    ...
    const style = translationStyle || 'translation-original';
  ...
  /**
   * 兼容旧接口（向后兼容，默认样式）
   * @deprecated 建议使用createWrapperElement
   */
  createReplacementElement(original, translation, phonetic, difficulty) {
    return this.createWrapperElement(original, translation, phonetic, difficulty, 'translation-original');
  }
